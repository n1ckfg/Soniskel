  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<settings.data.length;i++) {
        if (settings.data[i].equals("Mirror Tracking")) mirror = setBoolean(settings.data[i+1]);
        //if (settings.data[i].equals("Skeleton Tracking")) skelTrack = setBoolean(settings.data[i+1]);
